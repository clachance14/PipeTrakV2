-- Migration: Add Component Validation to Weld Completion Trigger
-- Description: Add validation check to prevent silent failure when component doesn't exist
-- Author: Claude Code
-- Date: 2025-11-20

-- =====================================================
-- Recreate trigger function with component validation
-- =====================================================
...
BEGIN
  ...
  IF OLD.date_welded IS NULL AND NEW.date_welded IS NOT NULL THEN
    ...
    -- Validate component exists before creating needs_review entry
    IF NOT EXISTS (SELECT 1 FROM components WHERE id = NEW.component_id) THEN
      RAISE WARNING 'Component % not found for weld %, skipping needs_review creation',
...
'Automatically creates needs_review entry when date_welded is set on a field weld. Only fires on NULL → NOT NULL transition to prevent duplicates. Validates component exists before creating needs_review entry. Allows service-role operations (imports, demo population) to bypass auth check.';
