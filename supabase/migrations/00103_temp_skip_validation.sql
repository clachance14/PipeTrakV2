-- Migration: Temporarily skip validation to test if UPDATE works
-- Feature: 026-editable-milestone-templates
-- Description: Remove validation to see if data is correct
...
BEGIN
  ...
  -- Return the calculated sum in the response so we can see what it was
  RETURN json_build_object(
    'success', true,
    'affected_count', affected_count,
    'audit_id', audit_id,
    'debug_sum', weight_sum,
    'debug_count', array_count
  );
END;
$$;

COMMENT ON FUNCTION update_project_template_weights IS
'Updates template weights WITHOUT validation (DEBUG VERSION)';
