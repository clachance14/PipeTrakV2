-- Migration: Fix progress delta to include field welds
--
-- Issue: Delta RPC excluded field_welds (component_type != 'field_weld') but
--        All Time view includes ALL components. This caused mismatch in budgets
--        and earned values for projects <30 days old (where delta should equal All Time).
--
-- Fix: Remove the field_weld exclusion to match All Time view calculations.
--
-- Before:
--   Delta: budget=2587.46, earned=73.63 (excluding 128 field_welds)
--   All Time: budget=3199.99, earned=175.03 (including field_welds)
--
-- After:
--   Both should match for projects less than 30 days old.
...
BEGIN
  ...
  WITH all_components AS (
    -- ALL non-retired components (INCLUDING field_welds to match All Time view)
    SELECT
      c.id,
      CASE p_dimension
    ...
    WHERE c.project_id = p_project_id
      ...
      -- Removed: AND c.component_type != 'field_weld'
  ),
  dimension_totals AS (
...
Includes ALL component types (including field_welds) to match All Time view.
