-- Migration: Remove optimistic locking to test if that's causing the issue
-- Feature: 026-editable-milestone-templates
-- Description: Temporarily disable optimistic locking AND validation
...
BEGIN
  ...
  -- SKIP optimistic locking check temporarily
  -- SKIP validation check temporarily
  ...
  RETURN json_build_object(
    ...
    'audit_id', audit_id,
    'debug_received_sum', weight_sum,
    'debug_array_count', array_count
  );
END;
$$;

COMMENT ON FUNCTION update_project_template_weights IS
'Updates template weights WITHOUT validation or optimistic locking (DEBUG VERSION)';
