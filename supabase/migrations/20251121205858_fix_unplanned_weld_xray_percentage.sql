-- Fix: Add xray_percentage to create_unplanned_weld RPC
--
-- Issue: Unplanned welds created without xray_percentage don't appear
--        in welder summary report (report filters by xray tier: 5%, 10%, 100%)
--
-- Solution:
--   1. Backfill existing unplanned welds with NULL xray_percentage (set to 5.0)
--   2. Update RPC to include xray_percentage parameter (default 5.0)
...
-- ============================================================================
-- Step 2: Update create_unplanned_weld RPC
-- ============================================================================

-- Drop old function signature (9 parameters)
DROP FUNCTION IF EXISTS create_unplanned_weld(UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT);

-- Create new function with xray_percentage parameter (10 parameters)
CREATE FUNCTION create_unplanned_weld(
  ...
  p_xray_percentage NUMERIC(5,2) DEFAULT 5.0  -- NEW: Default to 5% x-ray
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO field_welds (
    ...
    schedule,
    base_metal,
    xray_percentage,  -- NEW: Include xray_percentage
    notes,
    nde_required,
    status,
    created_by,
    ...
    p_schedule,
    p_base_metal,
    p_xray_percentage,  -- NEW: Set xray_percentage (defaults to 5.0)
    p_notes,
...
Created: 2025-11-17
Updated: 2025-11-21 (added xray_percentage parameter)
';
