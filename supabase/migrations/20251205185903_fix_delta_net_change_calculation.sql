-- Migration: Fix delta report to calculate NET change instead of summing all events
-- Feature: 033-timeline-report-filter
--
-- Problem: Previous implementation summed all milestone events within the date range.
-- When previous_value is NULL (for "complete" action events), it treated null -> X as +X change.
-- This overcounts when multiple events occur for the same milestone on the same component.
--
-- Solution: Calculate NET change per component/milestone pair:
--   - Find the FIRST event in range to get the "start" value (use previous_value if available, else 0)
--   - Find the LAST event in range to get the "end" value
--   - Net change = end_value - start_value
--
-- This correctly handles:
--   - Multiple events on same component (only counts net change)
--   - Events with NULL previous_value (treats as 0 start)
--   - Rollbacks (negative changes when end < start)
...
BEGIN
  ...
  net_changes AS (
    SELECT
      ...
      COALESCE(fle.end_value, 0) - COALESCE(fle.start_value, 0) AS net_value_change,
