-- Migration: Fix manhour budget permission check
-- The previous migration used `project_members` which doesn't exist
-- This project uses `user_organizations` for role-based permissions

-- Drop and recreate the function with correct permission check
...
BEGIN
  ...
  -- Uses user_organizations table joined with projects
...
Fixed permission check to use user_organizations table (not project_members).
