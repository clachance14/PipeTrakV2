-- Migration: Improve Activity Log Message Readability
-- Changes:
-- 1. Component-first structure for better scannability
-- 2. Use "updated" verb for partial progress, "marked" for completion
-- 3. Remove previous value display to reduce clutter
-- 4. New format examples:
--    Partial: "Doyle Hickman updated Spool V-26C03-SPOOL2: Erect 1% on Drawing V-26C03"
--    Complete: "Doyle Hickman marked Spool V-26C03-SPOOL2: Erect complete on Drawing V-26C03"
...
SELECT
  ...
  -- Improved activity message formatting
  concat(
    u.full_name,
    -- Verb changes based on whether it's a completion or update
    CASE
      WHEN me.value = 100 THEN ' marked '
      ELSE ' updated '
    END,
    -- Component identity (now appears before milestone)
    ...
    me.milestone_name,
    ' ',
    -- Value display (just the percentage or "complete")
    CASE
      WHEN me.value = 100 THEN 'complete'
      ELSE concat(me.value::integer, '%')
    END,
    -- Drawing display logic (stays at the end)
    CASE
