-- Migration: Simplify Field Weld Delta RPC
-- Feature: Timeline Report Filter (033)
--
-- BEFORE: Complex first/last event logic, manual category mapping
-- AFTER: Use delta_mh and category columns from milestone_events
--
-- Key changes:
-- 1. Filter to component_type = 'field_weld' only
-- 2. Use pre-calculated category column (no more manual mapping)
-- 3. Count completions where value went from <100 to 100
-- 4. Use delta_mh for earned value calculations
-- 5. Field weld categories map to install:
--    - fitup milestone -> install category
--    - weld_complete milestone -> install category
...
BEGIN
  ...
  completions AS (
    -- Count completions: when value goes from <100 to 100
    SELECT
      ed.component_id,
      ed.milestone_name,
      ed.category,
  ...
  aggregated AS (
    SELECT
      ...
      -- Map milestone names to completion counts
      -- fitup and weld_complete both contribute to install category
      COALESCE(SUM(CASE WHEN dm.milestone_name = 'fitup' THEN dm.completed ELSE 0 END), 0)::INT AS fitup_count,
      COALESCE(SUM(CASE WHEN dm.milestone_name = 'weld_complete' THEN dm.completed ELSE 0 END), 0)::INT AS weld_complete_count,
      COALESCE(SUM(CASE WHEN dm.milestone_name = 'accepted' THEN dm.completed ELSE 0 END), 0)::INT AS accepted_count,
      -- Sum delta_mh for percentage calculation
  ...
  total_budget AS (
    -- Calculate total manhour budget for each dimension
    SELECT
      CASE p_dimension
        WHEN 'area' THEN c.area_id
        WHEN 'system' THEN c.system_id
  ...
  SELECT
    ...
    -- Calculate percentage: (total_delta_mh / mh_budget) * 100
    CASE
...
'Simplified field weld progress delta using delta_mh and category columns.
Uses pre-calculated milestone_events.delta_mh and milestone_events.category.
Counts completions where value went from <100 to 100.
Feature: Timeline Report Filter (033)';
