-- Migration: Add optional p_metadata parameter to update_component_milestone RPC
-- Issue: Rollback reasons need to be stored in milestone_events.metadata
-- Changes:
-- 1. Add optional p_metadata parameter (JSONB)
-- 2. Merge p_metadata with calculated metadata (percent_complete values)
-- 3. Pass merged metadata to milestone_events INSERT

-- Drop existing function
DROP FUNCTION IF EXISTS update_component_milestone(UUID, TEXT, NUMERIC, UUID);

-- Recreate function with optional metadata parameter
CREATE OR REPLACE FUNCTION update_component_milestone(
  p_component_id UUID,
  p_milestone_name TEXT,
  p_new_value NUMERIC,
  p_user_id UUID,
  p_metadata JSONB DEFAULT NULL  -- NEW: Optional client metadata (rollback reasons, etc.)
) RETURNS JSON AS $$
DECLARE
  v_component RECORD;
  v_template RECORD;
...
BEGIN
  ...
  -- Client metadata (rollback reasons, etc.) takes precedence, but we add percent_complete values
...
COMMENT ON FUNCTION update_component_milestone IS
  'Atomically updates a component milestone, recalculates progress, creates audit event with optional client metadata (rollback reasons), delta_mh and category, and refreshes drawing progress view. Uses 0-100 scale for all milestones (discrete: 0 or 100, partial: 0-100).';
