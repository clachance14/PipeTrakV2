-- Migration: Fix Field Weld % Complete Calculation
--
-- Issue: Views calculate pct_total as AVG(c.percent_complete), which is the earned value
--        percentage. But the field weld report needs a simple completion percentage:
--        (welds completed / total welds) * 100
--
-- Fix: Replace pct_total calculation in all 3 views to use:
--      (weld_complete_count / total_welds) * 100
--
-- This is a straightforward completion percentage, not earned value.
...
SELECT
  ...
  -- Overall completion (FIXED: simple percentage, not earned value)
  ROUND(
    CASE
      WHEN COUNT(fw.id) > 0 THEN
        (COUNT(CASE WHEN (c.current_milestones->>'Weld Complete')::numeric = 1 THEN 1 END)::numeric / COUNT(fw.id)::numeric) * 100
      ELSE 0
    END, 0
  ) AS pct_total,
