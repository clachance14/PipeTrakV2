-- Migration: Create debug log table for troubleshooting weight sum issue
-- Feature: 026-editable-milestone-templates
-- Description: Log what the RPC function actually receives

CREATE TABLE IF NOT EXISTS debug_weight_logs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  logged_at timestamptz DEFAULT now(),
  component_type text,
  received_weights jsonb,
  calculated_sum integer,
  array_count integer
);

-- Update RPC function to log received data
...
DECLARE
  ...
  array_count integer;
BEGIN
  -- Calculate weight sum FIRST before any validation
  SELECT
  ...
  IF weight_sum != 100 THEN
    RAISE EXCEPTION 'Milestone weights must sum to 100%% for component type % (current: %)', p_component_type, weight_sum;
  END IF;
...
'Updates template weights with validation, audit logging, and optional retroactive recalculation. (DEBUG VERSION with logging)';
