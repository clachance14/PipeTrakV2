-- Migration: Fix field weld delta RPC milestone name matching
--
-- BUG: get_field_weld_delta_by_dimension checked for 'fitup', 'weld_complete', 'accepted'
--      but actual milestone names are 'Fit-up', 'Weld Complete', 'Test'
-- RESULT: delta_fitup_count, delta_weld_complete_count, delta_accepted_count were always 0
-- FIX: Use correct milestone names from the progress templates
...
BEGIN
  ...
  aggregated AS (
    SELECT
      ...
      -- Use actual milestone names from progress templates
