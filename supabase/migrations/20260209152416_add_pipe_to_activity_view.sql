-- Migration: Add pipe component type to vw_recent_activity view
-- Issue: Pipe components show raw UUID ("Component <uuid>") instead of "Pipe <pipe_id>"
-- Fix: Add WHEN 'pipe' clause to the CASE statement mapping component types to labels
...
SELECT
  ...
  concat(
    ...
    CASE c.component_type
      ...
      WHEN 'pipe' THEN concat('Pipe ', c.identity_key->>'pipe_id')
