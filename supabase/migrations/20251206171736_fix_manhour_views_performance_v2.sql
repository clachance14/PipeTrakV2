-- Fix manhour views performance - the function-based approach is too slow
-- with RLS enabled (3610 function calls for 722 components times out)
--
-- Simpler approach: For accurate category-level earned values, we need to look at
-- which milestones are complete. But since the proportional approach was incorrect,
-- we'll use a more efficient method that directly computes the category earned pct
-- based on which milestones are complete, using CASE statements.

-- Drop old views and helper function
...
DROP FUNCTION IF EXISTS get_component_category_earned(UUID, TEXT, JSONB, NUMERIC, TEXT);

-- Create a materialized view to pre-compute template milestone info
-- This avoids repeated joins to the template tables
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_template_milestone_weights AS
...
-- Recreate vw_manhour_progress_by_area with optimized approach
-- Key: We compute category earned at the component level, then aggregate
...
-- Get template weights for each component (prefer project-specific, fall back to default)
...
-- Calculate earned weight per milestone
component_milestone_earned AS (
  SELECT
    ...
    ct.weight,
    ct.is_partial,
    -- Normalize milestone value to 0-100
    CASE
...
-- Aggregate to category level per component
...
-- Calculate MH earned per category
component_earned AS (
  SELECT
    cc.*,
    -- Category MH earned = mh * (category_weight/100) * (earned_weight/total_weight)
...
-- Create system view with same approach
...
SELECT s.id AS system_id, s.name AS system_name, s.project_id,
  COALESCE(SUM(ce.mh), 0) AS mh_budget,
  COALESCE(SUM(ce.mh * ce.receive_total_weight / 100.0), 0) AS receive_mh_budget,
  COALESCE(SUM(ce.mh * ce.install_total_weight / 100.0), 0) AS install_mh_budget,
  COALESCE(SUM(ce.mh * ce.punch_total_weight / 100.0), 0) AS punch_mh_budget,
...
-- Create test package view with same approach
...
SELECT tp.id AS test_package_id, tp.name AS test_package_name, tp.project_id,
  COALESCE(SUM(ce.mh), 0) AS mh_budget,
  COALESCE(SUM(ce.mh * ce.receive_total_weight / 100.0), 0) AS receive_mh_budget,
  COALESCE(SUM(ce.mh * ce.install_total_weight / 100.0), 0) AS install_mh_budget,
  COALESCE(SUM(ce.mh * ce.punch_total_weight / 100.0), 0) AS punch_mh_budget,
