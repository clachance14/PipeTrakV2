-- Create RPC Function for Dynamic Weld Summary by Welder
-- Returns tier-grouped metrics (5%, 10%, 100% x-ray) with filtering capabilities
...
BEGIN
  ...
  SELECT
    ...
    END) AS welds_5pct,
    COUNT(CASE
      WHEN fw.xray_percentage = 5.0 AND fw.date_welded IS NOT NULL AND fw.nde_result IS NOT NULL
      THEN 1
    END) AS nde_5pct,
    COUNT(CASE
      WHEN fw.xray_percentage = 5.0 AND fw.date_welded IS NOT NULL AND fw.nde_result = 'FAIL'
      THEN 1
    END) AS reject_5pct,
    ...
    END) AS welds_10pct,
    COUNT(CASE
      WHEN fw.xray_percentage = 10.0 AND fw.date_welded IS NOT NULL AND fw.nde_result IS NOT NULL
      THEN 1
    END) AS nde_10pct,
    COUNT(CASE
      WHEN fw.xray_percentage = 10.0 AND fw.date_welded IS NOT NULL AND fw.nde_result = 'FAIL'
      THEN 1
    END) AS reject_10pct,
    ...
    END) AS welds_100pct,
    COUNT(CASE
      WHEN fw.xray_percentage = 100.0 AND fw.date_welded IS NOT NULL AND fw.nde_result IS NOT NULL
      THEN 1
    END) AS nde_100pct,
    COUNT(CASE
      WHEN fw.xray_percentage = 100.0 AND fw.date_welded IS NOT NULL AND fw.nde_result = 'FAIL'
      THEN 1
    ...
    END) AS welds_total,
    COUNT(CASE
      WHEN fw.date_welded IS NOT NULL AND fw.nde_result IS NOT NULL
      THEN 1
    END) AS nde_total,
    COUNT(CASE
      WHEN fw.date_welded IS NOT NULL AND fw.nde_result = 'FAIL'
      THEN 1
    END) AS reject_total,
    ...
    ROUND(
      CASE
        WHEN COUNT(CASE WHEN fw.date_welded IS NOT NULL AND fw.nde_result IS NOT NULL THEN 1 END) > 0 THEN
          (COUNT(CASE WHEN fw.date_welded IS NOT NULL AND fw.nde_result = 'FAIL' THEN 1 END)::numeric /
           COUNT(CASE WHEN fw.date_welded IS NOT NULL AND fw.nde_result IS NOT NULL THEN 1 END)::numeric) * 100
        ELSE 0
      END, 2
    ) AS reject_rate,

    -- NDE Completion % per tier (what % of completed welds in each tier have been x-rayed)
    ROUND(
      CASE
        WHEN COUNT(CASE WHEN fw.xray_percentage = 5.0 AND fw.date_welded IS NOT NULL THEN 1 END) > 0 THEN
          (COUNT(CASE WHEN fw.xray_percentage = 5.0 AND fw.date_welded IS NOT NULL AND fw.nde_result IS NOT NULL THEN 1 END)::numeric /
    ...
    ROUND(
      CASE
        WHEN COUNT(CASE WHEN fw.xray_percentage = 10.0 AND fw.date_welded IS NOT NULL THEN 1 END) > 0 THEN
          (COUNT(CASE WHEN fw.xray_percentage = 10.0 AND fw.date_welded IS NOT NULL AND fw.nde_result IS NOT NULL THEN 1 END)::numeric /
    ...
    ROUND(
      CASE
        WHEN COUNT(CASE WHEN fw.xray_percentage = 100.0 AND fw.date_welded IS NOT NULL THEN 1 END) > 0 THEN
          (COUNT(CASE WHEN fw.xray_percentage = 100.0 AND fw.date_welded IS NOT NULL AND fw.nde_result IS NOT NULL THEN 1 END)::numeric /
...
Returns completed welds (date_welded IS NOT NULL), NDE counts, and rejection counts per tier.
Supports filtering by date range, welders, and project dimensions (area, system, package).';

GRANT EXECUTE ON FUNCTION get_weld_summary_by_welder TO authenticated;
