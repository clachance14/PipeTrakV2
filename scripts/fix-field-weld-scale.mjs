/**
 * Fix field weld milestones to use 0/1 scale (what the views expect)
 */
...
async function main() {
  ...
  console.log(`Updating ${components.length} field weld components to use 0/1 scale...`)
  ...
  for (const c of components) {
    ...
    // Convert 100 -> 1, anything else -> 0 for each milestone
    for (const [key, value] of Object.entries(oldMs)) {
      newMs[key] = value >= 100 ? 1 : 0
    }
  ...
  viewData.forEach(r => {
    console.log(`${r.system_name}: ${r.pct_fitup}% fitup, ${r.pct_weld_complete}% weld complete, total: ${r.total_welds}`)
  })
}
